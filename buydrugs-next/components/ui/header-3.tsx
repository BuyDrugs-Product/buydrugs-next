  ...
  return (
    <header
      className={cn('sticky top-0 z-50 w-full border-b border-transparent', {
        'bg-(--surface-elevated) border-(--border-subtle) shadow-(--shadow-1)':
          scrolled,
      })}
    >
      <nav className="mx-auto flex h-14 w-full max-w-5xl items-center justify-between px-4">
        <div className="flex items-center gap-5">
          <a
            href="#"
            className="rounded-sm p-2 text-(--text-primary) transition hover:bg-(--state-hover)"
          >
          ...
          <NavigationMenu className="hidden md:flex">
            <NavigationMenuList>
              <NavigationMenuItem>
                <NavigationMenuTrigger className="bg-transparent text-(--text-secondary)">
                  Product
                </NavigationMenuTrigger>
                <NavigationMenuContent className="bg-(--surface-elevated) p-1 pr-1.5">
                  <ul className="grid w-lg grid-cols-2 gap-2 rounded-md border border-(--border-subtle) bg-(--surface-elevated) p-2 shadow-(--shadow-card)">
              ...
              <NavigationMenuItem>
                ...
                <NavigationMenuContent className="bg-(--surface-elevated) p-1 pr-1.5 pb-1.5">
                  <div className="grid w-lg grid-cols-2 gap-2">
                    <ul className="space-y-2 rounded-md border border-(--border-subtle) bg-(--surface-elevated) p-2 shadow-(--shadow-card)">
                    ...
                    <ul className="space-y-2 p-3">
                      {companyLinks2.map((item, i) => (
                        <li key={i}>
                          <NavigationMenuLink
                            href={item.href}
                            className="flex flex-row items-center gap-x-2 rounded-sm p-2 text-(--text-secondary) transition hover:bg-(--state-hover) hover:text-(--text-primary)"
                          >
                            <item.icon className="size-4 text-(--text-primary)" />
              ...
              <NavigationMenuLink className="px-4" asChild>
                <a
                  href="#"
                  className="rounded-sm p-2 text-(--text-secondary) transition hover:bg-(--state-hover) hover:text-(--text-primary)"
                >
                  Pricing
                </a>
              </NavigationMenuLink>
      ...
      <MobileMenu open={open} className="flex flex-col justify-between gap-2 overflow-y-auto">
        <NavigationMenu className="max-w-full">
          <div className="flex w-full flex-col gap-y-2">
            ...
            <span className="mt-2 text-xs font-semibold uppercase tracking-[0.2em] text-(--text-tertiary)">
              Company
            </span>
...
function ListItem({
  ...
  return (
    <NavigationMenuLink
      className={cn(
        'flex w-full flex-row gap-x-2 rounded-sm p-2 text-(--text-secondary) transition',
        'hover:bg-(--state-hover) hover:text-(--text-primary) focus:bg-(--state-selected) focus:text-(--text-primary)',
        className,
      )}
      {...props}
      asChild
    >
      <a href={href}>
        <div className="flex aspect-square size-12 items-center justify-center rounded-md border border-(--border-subtle) bg-(--surface-elevated) shadow-(--shadow-1)">
